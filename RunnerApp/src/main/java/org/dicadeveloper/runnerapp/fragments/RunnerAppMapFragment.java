...
import org.dicadeveloper.runnerapp.MapOverlay;
...
import org.dicadeveloper.runnerapp.content.TrackDataHub;
import org.dicadeveloper.runnerapp.content.TrackDataListener;
import org.dicadeveloper.runnerapp.content.TrackDataType;
import org.dicadeveloper.runnerapp.content.Waypoint;
import org.dicadeveloper.runnerapp.maps.OsmdroidMapWrapper;
...
/**
 * Created by itdog on 10.02.15.
 */
public class RunnerAppMapFragment extends SupportMapFragment implements TrackDataListener {
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
     ...
     * initialize mMap. However, onActivityCreated can be called multiple
     * times, e.g., when the user switches tabs. With
     * GoogleMapOptions.useViewLifecycleInFragment == false, mMap lifecycle
     * is tied to the fragment lifecycle and the same mMap object is
     * returned in getMap. Thus we only need to initialize mMap once, when
     * it is null.
     */
    if (mMap == null) {
      final org.osmdroid.views.MapView mapView = new org.osmdroid.views.MapView(getActivity(), 256);
      mapView.setBuiltInZoomControls(true);
      //setContentView(mapView);
      mMap = new OsmdroidMapWrapper(mapView);
      ...
      //TODO: Implement location source
      /*
  ...
  public void onResume() {
    ...
    if (mMap != null) {
      ...
      mMap.setMyLocationEnabled(isGpsProviderEnabled);
    }
  ...
  public void onNewTrackPointsDone() {
    if (isResumed()) {
      getActivity().runOnUiThread(new Runnable() {
        public void run() {
          if (isResumed() && mMap != null && currentTrack != null) {
            boolean hasStartMarker = mapOverlay.update(
                    mMap, paths, currentTrack.getTripStatistics(), reloadPaths);
  ...
  public void onNewWaypointsDone() {
    if (isResumed()) {
      getActivity().runOnUiThread(new Runnable() {
        public void run() {
          if (isResumed() && mMap != null && currentTrack != null) {
            mapOverlay.update(mMap, paths, currentTrack.getTripStatistics(), true);
          }
        }
      });
    }
  ...
  private void updateCurrentLocation(final boolean forceZoom) {
    getActivity().runOnUiThread(new Runnable() {
      public void run() {
        if (!isResumed() || mMap == null || onLocationChangedListener == null
                || currentLocation == null) {
          return;
        }
  ...
  private void showTrack() {
    if (mMap == null || currentTrack == null) {
      return;
    }
    if (currentTrack.getNumberOfPoints() < 2) {
      //TODO: Implement Camera update
      //mMap.moveCamera(
      //        CameraUpdateFactory.newLatLngZoom(getDefaultLatLng(), mMap.getMinZoomLevel()));
      return;
    }
  ...
  private void moveCameraOverTrack() {
     ...
     */
    if (mMap == null || currentTrack == null || currentTrack.getNumberOfPoints() < 2
  ...
  private void showMarker(final long id) {
    getActivity().runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        MyTracksProviderUtils MyTracksProviderUtils = org.dicadeveloper.runnerapp.content.MyTracksProviderUtils.Factory.get(getActivity());
